...
//=================================================================================================================================================================
//? Decorating class for the screen 
//=================================================================================================================================================================
class Decorating{
public:
...
class Interface {
public:
    void draw_result(int score, string time, string time_unit, string time_unit2, Game& game);
    Decorating style;
...
void Interface::draw_result(int score, string time, string time_unit, string time_unit2, Game& game) {
    ...
    while (true) {
        ...
        for (int i = 0; i < congrats.size(); ++i) {
            COORD pos = {static_cast<SHORT>(start_x), static_cast<SHORT>(start_y + i)};
            SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), pos);
        ...
        COORD result_pos = {static_cast<SHORT>(start_result_x), static_cast<SHORT>(start_y * 2)};
        SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), result_pos);
        cout << result << endl;

        // display play again option
        string x = "Well played! Give it another shot? (press 's')";
        int play_again_x = (console_width - x.size()) / 2;
        COORD play_again = {static_cast<SHORT>(play_again_x), static_cast<SHORT>(start_y * 2 + 2)};
        SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), play_again);
        cout << x << endl;

        // display quit option
        string y = "Quit the Game (press 'q')";
        int quit_x = (console_width - y.size()) / 2;
        COORD quit = {static_cast<SHORT>(quit_x), static_cast<SHORT>(start_y * 2 + 4)};
        SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), quit);
        cout << y << endl;
