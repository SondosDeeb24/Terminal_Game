...
public:
    ...
    void move_left(Map &map) {
        ...
        if (next >= 0) {
            if (is_jumping) {
                if (map.get_above(next) == '_') pos = next;
            } else {
                if (map.get_ground(next) == '_') pos = next;
                else if (map.get_ground(pos) == '#' && map.get_ground(next) == '#') pos = next; // _##_ case and user on the second block , so let him jump on the previous block
    ...
    void move_right(Map &map, bool &finish_line_reached) {
        ...
        if (next <= map_limit) {
            if (is_jumping) {
                ...
                if (map.get_above(next) == '_') pos = next;// here if the block upon him is line and he is jumping, then let me replace it
                ...
                else if (map.get_ground(next) == '_') pos = next;
