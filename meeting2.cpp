...
public:
    ...
    void draw(int pos, bool is_jumping) { //pos = the current location for the player
        ...
        for (int i = 0; i < 2 ; i ++){
            COORD map_position = {static_cast<SHORT>(draw_map_x), static_cast<SHORT>(draw_map_y * 2  + i)};
...
public:
    ...
    void draw_clock() {
        ...
        int draw_clock_x = (console_width - size(time_layout[0]) )/ 2;
        int draw_clock_y = (console_height - size(time_layout[0]) )/ 3;

        for (int i = 0; i < size(time_layout) ; i ++){
            COORD timer_pos = {static_cast<SHORT>(draw_clock_x), static_cast<SHORT>(draw_clock_y + i)};
